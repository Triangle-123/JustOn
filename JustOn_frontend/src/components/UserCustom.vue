<template>
    <button
      :class="switchStore.isKicked ? 'kick2' : ''"
      class="custom absolute left-0 top-0 p-4 w-[100px] h-[100px] bg-[var(--juston-black)] rounded-[50%] text-xl font-bold text-white"
    ...
    >
      ...
      <div class="flex justify-center items-center">
        <div
          class="bottom-[150p] bg-[#f7f7f7] w-[400px] py-12 px-8 rounded-[16px] flex justify-center items-center flex-col"
          v-click-outside="closeModal"
        >
          <span class="px-6 pb-2 mr-1 self-center inline-block"
            ><strong class="text-4xl font-[800]">배경색 변경</strong></span
          >
          ...
          <div class="flex flex-row">
            <div
              class="flex flex-col justify-center items-center px-3 border-solid border-r border-gray-500"
            >
              <span>현재 색상</span>
  ...
  const setCurGradient = () => {
    if (userStore.user.color) {
      ...
      document.documentElement.style.setProperty(
        "--after-gradient-color1",
        color1.value
      );
      document.documentElement.style.setProperty(
        "--after-gradient-color2",
        color2.value
      );
      document.documentElement.style.setProperty(
        "--after-gradient-color3",
        color3.value
      );
    }
  };
