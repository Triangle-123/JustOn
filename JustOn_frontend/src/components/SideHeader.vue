<template>
  ...
  >
    <div class="top-logo pt-8 px-10 mb-11 flex justify-between">
      <a class="logo flex items-end cursor-pointer" @click="navigateTo('home')">
        <span class="mr-2 leading-[16px]">날 위한 운동 메이트</span>
        <img class="inline-block h-[30px]" src="../assets/logo.svg" alt="" />
      </a>
      <i
        @click="
          rotate = !rotate;
        ...
        class="bi bi-arrow-left-circle cursor-pointer text-3xl flex justify-center"
      ></i>
    </div>
    ...
    <div class="profile mx-6 bg-[--juston-bg-w] p-5 rounded-2xl gap-3">
      <div class="profile-img relative">
        <div
          class="w-20 h-20 rounded-[50%] overflow-hidden flex items-center justify-center"
        >
          <img
        ...
        />
        <label
          class="absolute bottom-0 right-0 cursor-pointer text-base"
          for="proImg-upload"
          v-if="userStore.user"
          ><i
...
const sMenus = [
  { iconClass: "bi bi-house-heart", text: "홈", routerName: "home" },
  {
